    ...
    public static void openManageGui(Player p,String type){
        ...
        switch (types.get(0)) {
                    ...
                    case "admin":
                        ...
                        switch (types.get(3)) {
                            ...
                            case "permission":
                                ...
                                switch (t4) {
                                    case "def":
                                        Set<String> uuidList = region.getConfigurationSection(id + ".user") != null ? region.getConfigurationSection(id + ".user").getKeys(false) : new HashSet<>();
                                    ...
                                    default:
                                        ...
                                        inv = setDefaultPermissionGui(inv, id, t4);

                                        if (!isOwner(p, id))
                                            inv.setItem(START_PERM_PLACE, createItem(Material.BARRIER, "§c§l変更権限なし", null));
                            ...
                            case "status":
                                ...
                                if (p.isOp())
                                    inv = setItemPlus(inv, createItem(Material.BLUE_ICE, "§c§l凍結する", List.of("§f土地を凍結すると、オーナー", "§fも土地を触れなくなります")), 9, 10, 11, 12, 13, 14, 15, 16, 17);
                                break;
                        }
                }
                break;
            ...
            case "world":
                ...
                String worldUUID = Bukkit.getWorld(id).getUID().toString();
                switch (types.get(2)){
                    case "def":
                        if(world.get(worldUUID)==null) {
                            world.set(worldUUID + ".permission", 64);
                            world.set(worldUUID + ".enabled", false);
                    ...
                    case "permission":
                        ...
                        inv = setDefaultPermissionGui(inv, worldUUID,null);
